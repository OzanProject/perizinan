...
<?php $__env->startSection('title', 'Editor Template Sertifikat'); ?>
<?php $__env->startSection('breadcrumb', 'Desain Template'); ?>

<?php $__env->startSection('content'); ?>
  <?php $__env->startPush('styles'); ?>
    <link rel="stylesheet" href="<?php echo e(asset('css/template-editor.css')); ?>?v=<?php echo e(time()); ?>">
  <?php $__env->stopPush(); ?>
  <div class="container-fluid text-dark">
    ...
    <div class="row mb-3 align-items-center">
      <div class="col-sm-6 text-center text-sm-left">
        ...
        <p class="text-muted small mt-1">Desain Template: <?php echo e($jenisPerizinan->nama); ?></p>
      </div>
    </div>
    ...
    <div class="card card-outline card-primary shadow-sm border-0">
      ...
      <div class="card-header bg-white py-2 d-flex justify-content-between align-items-center flex-wrap">
        ...
        <div class="d-flex align-items-center ml-auto flex-wrap">
          ...
          <div class="custom-control custom-switch custom-switch-off-light custom-switch-on-success mr-2 mt-1"
            ...
            <input type="checkbox" class="custom-control-input" id="use-watermark-checkbox" <?php echo e(($jenisPerizinan->use_watermark ?? false) ? 'checked' : ''); ?>>
          ...
          <div class="custom-control custom-switch custom-switch-off-light custom-switch-on-success mr-3 mt-1"
            ...
            <input type="checkbox" class="custom-control-input" id="use-border-checkbox" <?php echo e(($jenisPerizinan->use_border ?? false) ? 'checked' : ''); ?>>
          ...
          <div class="btn-group btn-group-sm mr-3" id="frame-selector-group"
            style="display: <?php echo e(($jenisPerizinan->use_border ?? false) ? 'flex' : 'none'); ?>;">
          ...
          <select class="form-control form-control-sm font-weight-bold text-dark mr-2" id="paper-size-selector"
            style="width: 150px;">
            <option value="a4" <?php echo e(strtoupper($jenisPerizinan->paper_size ?? $activePreset->paper_size ?? 'A4') == 'A4' ? 'selected' : ''); ?>>A4 (21 x 29.7 cm)</option>
            <option value="f4" <?php echo e(strtoupper($jenisPerizinan->paper_size ?? $activePreset->paper_size ?? 'A4') == 'F4' ? 'selected' : ''); ?>>F4 / Folio (21.5 x 33 cm)</option>
          </select>

          <select class="form-control form-control-sm font-weight-bold text-dark mr-3" id="paper-orientation"
            style="width: 120px;">
            <option value="portrait" <?php echo e(strtolower($jenisPerizinan->orientation ?? $activePreset->orientation ?? 'portrait') == 'portrait' ? 'selected' : ''); ?>>Portrait</option>
            <option value="landscape" <?php echo e(strtolower($jenisPerizinan->orientation ?? $activePreset->orientation ?? 'portrait') == 'landscape' ? 'selected' : ''); ?>>Landscape</option>
          </select>
      ...
      <div class="card-body p-0 d-flex flex-column overflow-hidden" style="background-color: #e9ecef;">
        ...
        <div class="overflow-auto custom-scrollbar d-flex justify-content-center py-5" id="workspace-container"
          ...
          <form id="template-form" action="<?php echo e(route('super_admin.jenis_perizinan.template.update', $jenisPerizinan)); ?>"
            method="POST">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="template_html" id="template-input">
            <input type="hidden" name="use_border" id="use-border-input"
              value="<?php echo e($jenisPerizinan->use_border ? '1' : '0'); ?>">
            <input type="hidden" name="border_type" id="border-type-input"
              value="<?php echo e(old('border_type', $jenisPerizinan->border_type)); ?>">
            <input type="hidden" name="use_watermark" id="use-watermark-input"
              value="<?php echo e($jenisPerizinan->use_watermark ? '1' : '0'); ?>">
            <input type="hidden" name="paper_size" id="paper-size-input"
              value="<?php echo e(old('paper_size', $jenisPerizinan->paper_size ?? $activePreset->paper_size ?? 'A4')); ?>">
            <input type="hidden" name="orientation" id="orientation-input"
              value="<?php echo e(old('orientation', $jenisPerizinan->orientation ?? $activePreset->orientation ?? 'portrait')); ?>">
            ...
            <div id="document-wrapper" class="shadow-lg position-relative mx-3"
              ...
              <div id="frame-overlay" style="
                  ...
                  background-image: url('<?php echo e($overlayUrl); ?>');
                  background-size: 100% 100%; background-repeat: no-repeat;
                  opacity: <?php echo e($dinas->watermark_border_opacity ?? 0.9); ?>;
                  display: <?php echo e(($jenisPerizinan->use_border ?? false) ? 'block' : 'none'); ?>;
              "></div>

              <div id="watermark-overlay" style="
                  position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);
                  pointer-events: none; z-index: 11;
                  opacity: <?php echo e($watermarkOpacity); ?>;
                  display: <?php echo e(($jenisPerizinan->use_watermark ?? false) ? 'block' : 'none'); ?>;
              ">
                  <img src="<?php echo e($watermarkUrl); ?>" style="width: 300px; height: auto; object-fit: contain;">
              </div>

              <div id="editor-canvas" class="document-editor__editable paper-a4-portrait" contenteditable="true">
                <?php echo $jenisPerizinan->template_html ?? '<div style="text-align:center; padding-top:50px; color:#ccc;"><h2>Kanvas Kosong</h2><p>Klik tombol kuning "Pilih Layout" di atas untuk memulai desain secara otomatis.</p></div>'; ?>
          ...
          <div id="variable-content" class="p-3 bg-light custom-scrollbar" style="max-height: 25vh; overflow-y: auto;">
            ...
            <div class="row">
              ...
              <div class="col-md-4 mb-3">
                ...
                <?php if($jenisPerizinan->form_config): ?>
                  <?php $__currentLoopData = $jenisPerizinan->form_config; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <button data-insert-var="[DATA:<?php echo e(strtoupper($field['name'])); ?>]"
                      class="btn btn-outline-dark btn-sm mb-1 mr-1 var-btn">
                      <i class="fas fa-check-circle text-success mr-1"></i> <?php echo e($field['label']); ?>
    ...
    <div class="modal-dialog modal-xl" role="document">
      <div class="modal-content border-0 shadow-lg">
        ...
        <div class="modal-body p-4 bg-light">
          <div class="row">
            <?php if(isset($presets) && is_array($presets)): ?>
              <?php $__currentLoopData = $presets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $preset): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="col-md-4 mb-4">
                  <div class="card h-100 border-0 shadow-sm transition-all cursor-pointer btn-apply-preset" data-preset-key="<?php echo e($key); ?>" onclick="TemplateEditor.applyPreset('<?php echo e($key); ?>')"
                    ...
                    <div class="card-body p-0">
                      <div class="bg-white p-3 border-bottom overflow-hidden d-flex justify-content-center align-items-center"
                        ...
                        <i
                          class="fas <?php echo e(($preset['orientation'] ?? 'portrait') == 'landscape' ? 'fa-file-invoice' : 'fa-file-alt'); ?> fa-4x text-<?php echo e(($preset['use_border'] ?? false) ? 'warning' : 'primary'); ?> opacity-50"></i>
                      </div>
                      <div class="p-3">
                        <h6 class="font-weight-bold text-dark mb-1"><?php echo e($preset['name'] ?? 'Template'); ?></h6>
                        <p class="text-muted small mb-0"><?php echo e($preset['description'] ?? ''); ?></p>
                        <div class="mt-2">
                          <span class="badge badge-light border"><?php echo e($preset['paper_size'] ?? 'A4'); ?></span>
                          <span class="badge badge-light border"><?php echo e(ucfirst($preset['orientation'] ?? 'Portrait')); ?></span>
                          <?php if($preset['use_border'] ?? false): ?>
  ...
  <?php $__env->startPush('scripts'); ?>
    <script src="https://cdn.tiny.cloud/1/<?php echo e($dinas->tinymce_api_key ?: env('TINYMCE_API_KEY', 'no-api-key')); ?>/tinymce/7/tinymce.min.js"
      referrerpolicy="origin"></script>

    <script>
      window.TemplateEditorConfig = {
        tinymceApiKey: "<?php echo e($dinas->tinymce_api_key ?: env('TINYMCE_API_KEY', 'no-api-key')); ?>",
        logoUrl: <?php echo json_encode($logoUrl ?? '', 15, 512) ?>,
        presets: <?php echo json_encode($presets ?? [], 15, 512) ?>,
        savedBorderType: '<?php echo e($jenisPerizinan->border_type); ?>',
        namaIzin: '<?php echo e(strtolower($jenisPerizinan->nama ?? '')); ?>',
        framePaths: {
          'default': '<?php echo e($frameUrl ?? asset('images/default-border.png')); ?>',
          'paud': '<?php echo e($dinas->watermark_border_paud_img ? asset('storage/' . $dinas->watermark_border_paud_img) : asset('images/bingkai-paud.jpg')); ?>',
          'lkp': '<?php echo e($frameUrl ?? asset('images/default-border.png')); ?>'
        }
      };
    </script>
    <script src="<?php echo e(asset('js/template-editor.js')); ?>?v=<?php echo e(time()); ?>"></script>
  <?php $__env->stopPush(); ?>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.backend', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\perizinan\resources\views/backend/super_admin/jenis_perizinan/template.blade.php ENDPATH**/ ?>