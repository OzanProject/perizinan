    ...
    <div class="container-fluid py-4" x-data="wizard({{ $initialStep }})">
        ...
        <div class="row mb-4 align-items-center">
            <div class="col-md-9">
                ...
                <p class="text-muted small mb-0">Silakan ikuti langkah-langkah di bawah untuk melengkapi berkas pengajuan
                    <strong>{{ $perizinan->jenisPerizinan->nama }}</strong>.</p>
            </div>
            ...
            <div class="card-body p-4">
                <div class="row position-relative px-lg-5">
                    ...
                    <div class="col-4 text-center">
                        <button @click="goToStep(1)" class="step-item d-inline-flex flex-column align-items-center mx-auto"
                            :class="step >= 1 ? (step > 1 ? 'completed' : 'active') : ''">
                            <div class="step-circle mb-2">
                                <template x-if="step > 1"><i class="fas fa-check"></i></template>
                                <template x-if="step <= 1"><span>1</span></template>
                            </div>
                            <span class="small font-weight-bold text-uppercase d-none d-sm-block"
                                :class="step >= 1 ? 'text-dark' : 'text-muted'">Info Detail</span>
                        </button>
                    </div>

                    <!-- Step 2 -->
                    <div class="col-4 text-center">
                        <button @click="goToStep(2)" class="step-item d-inline-flex flex-column align-items-center mx-auto"
                            :class="step >= 2 ? (step > 2 ? 'completed' : 'active') : ''">
                            <div class="step-circle mb-2">
                                <template x-if="step > 2"><i class="fas fa-check"></i></template>
                                <template x-if="step <= 2"><span>2</span></template>
                            </div>
                            <span class="small font-weight-bold text-uppercase d-none d-sm-block"
                                :class="step >= 2 ? 'text-dark' : 'text-muted'">Upload Berkas</span>
                        </button>
                    </div>

                    <!-- Step 3 -->
                    <div class="col-4 text-center">
                        <button @click="goToStep(3)" class="step-item d-inline-flex flex-column align-items-center mx-auto"
                            :class="step >= 3 ? 'active' : ''">
                            <div class="step-circle mb-2">
                                <span>3</span>
                            </div>
                            <span class="small font-weight-bold text-uppercase d-none d-sm-block"
                                :class="step >= 3 ? 'text-dark' : 'text-muted'">Review & Kirim</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-8">
                ...
                <div x-show="step === 1" class="mb-4">
                    <div class="card card-outline card-primary shadow-sm">
                        ...
                        <form id="form-data-perizinan"
                            action="{{ route('admin_lembaga.perizinan.update_data', $perizinan) }}" method="POST">
                            @csrf
                            <div class="card-body p-4">
                                <p class="text-muted small mb-4">Informasi di bawah ini akan digunakan sebagai data
                                    identitas pada sertifikat izin Anda.</p>
                                ...
                                <div class="row">
                                        ...
                                        @foreach($perizinan->jenisPerizinan->form_config as $field)
                                            <div class="col-md-{{ ($field['type'] ?? 'text') == 'textarea' ? '12' : '6' }} mb-3">
                                                <div class="form-group">
                                                    <label
                                                        class="text-uppercase small font-weight-bold text-muted mb-2 tracking-wider">
                                                        {{ $field['label'] }} @if($field['required'] ?? false) <span
                                                        class="text-danger">*</span> @endif
                                                    </label>
                                                    @if(($field['type'] ?? 'text') == 'textarea')
                                                        <textarea name="data[{{ $field['name'] }}]"
                                                            class="form-control form-control-lg font-weight-bold" rows="3"
                                                            required>{{ $perizinan->perizinan_data[$field['name']] ?? '' }}</textarea>
                                                    @else
                                                        <input type="{{ $field['type'] ?? 'text' }}" name="data[{{ $field['name'] }}]"
                                                            value="{{ $perizinan->perizinan_data[$field['name']] ?? '' }}"
                                    ...
                                    @else
                                        <div class="col-12 text-center py-5">
                                            ...
                                            <p class="text-muted font-italic">Tidak ada data tambahan yang diperlukan untuk
                                                jenis izin ini.</p>
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="card-footer bg-light px-4 py-3 text-right">
                                <button type="button" @click="submitStep1()"
                                    class="btn btn-primary px-5 py-2 font-weight-bold shadow-sm">
                ...
                <div x-show="step === 2" class="mb-4" x-cloak>
                    <div class="card card-outline card-primary shadow-sm">
                        ...
                        <div class="card-body p-4">
                            <p class="text-muted small mb-4">Pastikan dokumen hasil scan jelas dan terbaca. Format: PDF,
                                JPG, PNG.</p>
                                    ...
                                    <div class="list-group-item px-0 py-4 border-light">
                                        <div class="row align-items-start">
                                            <div class="col-md-8">
                                                <div class="d-flex align-items-center mb-1">
                                                    ...
                                                    @if($syarat->is_required)
                                                        <span class="badge badge-danger text-uppercase p-1 px-2"
                                                            style="font-size: 9px;">Wajib</span>
                                                    @endif
                                                </div>
                                                <p class="text-muted small mb-3">
                                                    {{ $syarat->deskripsi ?? 'Silakan unggah dokumen yang sah untuk persyaratan ini.' }}
                                                </p>

                                                @if($dokumen)
                                                    <div
                                                        class="d-flex align-items-center p-3 rounded bg-light border border-success-light">
                                                        <div class="bg-white rounded p-2 mr-3 border shadow-sm">
                                                            <i class="fas fa-file-pdf text-danger fa-lg"></i>
                                                        </div>
                                                        <div class="flex-grow-1 min-w-0">
                                                            <span
                                                                class="d-block font-weight-bold text-truncate small">{{ $dokumen->nama_file }}</span>
                                                            <span class="text-success small font-weight-bold"><i
                                                                    class="fas fa-check-circle mr-1"></i> Terunggah</span>
                                                        </div>
                                                        <div class="ml-3">
                                                            <button
                                                                @click="openFilePreview('{{ Storage::url($dokumen->path) }}', '{{ $dokumen->nama_file }}')"
                                                                class="btn btn-sm btn-light border" title="Lihat">
                                                                <i class="fas fa-eye text-muted"></i>
                                                            </button>
                                                            <button
                                                                @click="deleteFile({{ $perizinan->id }}, {{ $dokumen->id }}, {{ $syarat->id }})"
                                                                class="btn btn-sm btn-light border ml-1" title="Hapus">
                                            ...
                                            <div class="col-md-4 text-md-right mt-3 mt-md-0">
                                                <input type="file" id="file-{{ $syarat->id }}" class="d-none"
                                                    @change="uploadFile($event, {{ $syarat->id }})">
                                                <button type="button"
                                                    @click="document.getElementById('file-{{ $syarat->id }}').click()"
                                                    class="btn btn-sm font-weight-bold text-uppercase px-3 py-2 shadow-sm"
                                                    :class="{{ $dokumen ? 'btn-outline-secondary' : 'btn-primary' }}">
                                                    <i class="fas {{ $dokumen ? 'fa-sync-alt' : 'fa-upload' }} mr-2"></i>
                                                    {{ $dokumen ? 'Ganti' : 'Unggah' }}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div x-show="step === 3" class="mb-4" x-cloak>
                    <div class="card card-outline card-success shadow-sm">
                        ...
                        <div class="card-body p-4">
                            <p class="text-muted small mb-4">Pastikan seluruh data dan berkas yang Anda unggah sudah benar
                                dan sesuai asli.</p>
                                        ...
                                        @if($perizinan->jenisPerizinan->form_config)
                                            @foreach($perizinan->jenisPerizinan->form_config as $field)
                                                <div class="col-md-6 mb-3">
                                                    <label
                                                        class="text-uppercase small font-weight-bold text-muted mb-1">{{ $field['label'] }}</label>
                                                    <div class="font-weight-bold text-dark">
                                                        {{ $perizinan->perizinan_data[$field['name']] ?? '-' }}</div>
                                                </div>
                                            @endforeach
                                        @endif
                                    </div>
                                    ...
                                    @foreach($syarats as $syarat)
                                        ...
                                        <div class="list-group-item d-flex align-items-center justify-content-between py-2">
                                            <div class="d-flex align-items-center">
                                                <i
                                                    class="fas {{ $dokumen ? 'fa-check-circle text-success' : 'fa-times-circle text-danger' }} mr-2"></i>
                                                <span class="small font-weight-bold">{{ $syarat->nama_dokumen }}</span>
                                            </div>
                                            <span
                                                class="badge font-weight-bold text-uppercase {{ $dokumen ? 'text-success' : 'text-danger' }}"
                                                style="font-size: 8px;">
                            ...
                            <div class="alert alert-warning border shadow-none small d-flex align-items-start">
                                ...
                                <div>
                                    Dengan menekan tombol <strong>Ajukan Sekarang</strong>, data akan dikirim ke sistem
                                    verifikasi Super Admin dan tidak dapat diubah hingga ada catatan perbaikan.
                                </div>
                            </div>
                        </div>
                        <div class="card-footer bg-light px-4 py-4 d-flex flex-column flex-sm-row justify-content-between">
                            <button type="button" @click="goToStep(2)"
                                class="btn btn-outline-secondary font-weight-bold px-4 mb-3 mb-sm-0">
                    ...
                    <div class="card-body p-4 text-center">
                        ...
                        <div class="position-relative d-inline-block mb-3">
                            <div class="progress-circle shadow-sm" style="width: 120px; height: 120px;">
                                <svg viewBox="0 0 36 36" class="w-100 h-100 rotate-n90">
                                    <circle cx="18" cy="18" r="16" fill="none" class="stroke-light" stroke-width="3">
                                    </circle>
                        ...
                        <div class="row text-center mt-3">
                            ...
                            <div class="col-6">
                                ...
                                <div class="h6 font-weight-bold text-danger mb-0">{{ $requiredCount - $uploadedCount }}
                                    Berkas</div>
                            </div>
                        </div>
                        ...
                        <button @click="document.getElementById('chat-panel').classList.toggle('open')"
                            class="btn btn-info btn-block py-2 font-weight-bold shadow-sm">
                    ...
                    <div class="card-body p-4 pt-2">
                        <ul class="list-unstyled small space-y-3">
                            <li class="d-flex align-items-start border-bottom pb-2 mb-2">
                                ...
                                <span>Gunakan file PDF hasil scan asli, bukan foto kamera HP jika memungkinkan agar lebih
                                    profesional.</span>
                            </li>
                            <li class="d-flex align-items-start border-bottom pb-2 mb-2">
                                <i class="fas fa-file-contract text-info mr-3 mt-1"></i>
                                <span>Satu jenis berkas harus digabung ke dalam satu file PDF jika terdiri dari beberapa
                                    halaman.</span>
                            </li>
                        </ul>
                    </div>
            ...
            <div class="card-header rounded-0 bg-primary d-flex align-items-center py-3">
                ...
                <div class="card-tools ml-auto">
                    <button type="button" class="btn btn-tool text-white"
                        @click="document.getElementById('chat-panel').classList.remove('open')">
                    ...
                    @forelse($perizinan->discussions as $chat)
                        <div class="direct-chat-msg {{ $chat->user_id == Auth::id() ? 'right' : '' }} mb-4">
                            <div class="direct-chat-infos clearfix mb-1">
                                <span
                                    class="direct-chat-name {{ $chat->user_id == Auth::id() ? 'float-right' : 'float-left' }} font-weight-bold small uppercase">{{ $chat->user->name }}</span>
                                <span
                                    class="direct-chat-timestamp {{ $chat->user_id == Auth::id() ? 'float-left' : 'float-right' }} small">{{ $chat->created_at->format('H:i, d M') }}</span>
                            </div>
                            <div class="direct-chat-text shadow-sm border-0 py-2 px-3 {{ $chat->user_id == Auth::id() ? 'bg-primary text-white' : 'bg-white text-dark' }}"
            ...
            <div class="card-footer bg-white border-top p-4">
                <form action="{{ route('admin_lembaga.perizinan.discussion.store', $perizinan) }}" method="POST">
                    ...
                    <div class="input-group">
                        <input type="text" name="message" placeholder="Tanyakan kendala Anda..."
                            class="form-control form-control-lg border-light bg-light" required>
